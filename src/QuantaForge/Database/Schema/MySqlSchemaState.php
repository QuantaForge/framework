...
namespace QuantaForge\Database\Schema;
...
class MySqlSchemaState extends SchemaState
{
    ...
    public function dump(Connection $connection, $path)
    {
        $this->executeDumpProcess($this->makeProcess(
            $this->baseDumpCommand().' --routines --result-file="${:QUANTAFORGE_LOAD_PATH}" --no-data'
        ), $this->output, array_merge($this->baseVariables($this->connection->getConfig()), [
            'QUANTAFORGE_LOAD_PATH' => $path,
        ]));
    ...
    {
        $command = 'mysql '.$this->connectionString().' --database="${:QUANTAFORGE_LOAD_DATABASE}" < "${:QUANTAFORGE_LOAD_PATH}"';
    ...
    protected function baseDumpCommand()
    {
        ...
        return $command.' "${:QUANTAFORGE_LOAD_DATABASE}"';
    }
    ...
    {
        $value = ' --user="${:QUANTAFORGE_LOAD_USER}" --password="${:QUANTAFORGE_LOAD_PASSWORD}"';

        $config = $this->connection->getConfig();

        $value .= $config['unix_socket'] ?? false
                        ? ' --socket="${:QUANTAFORGE_LOAD_SOCKET}"'
                        : ' --host="${:QUANTAFORGE_LOAD_HOST}" --port="${:QUANTAFORGE_LOAD_PORT}"';

        if (isset($config['options'][\PDO::MYSQL_ATTR_SSL_CA])) {
            $value .= ' --ssl-ca="${:QUANTAFORGE_LOAD_SSL_CA}"';
        }
    ...
    protected function baseVariables(array $config)
    {
        ...
        return [
            'QUANTAFORGE_LOAD_SOCKET' => $config['unix_socket'] ?? '',
            'QUANTAFORGE_LOAD_HOST' => is_array($config['host']) ? $config['host'][0] : $config['host'],
            'QUANTAFORGE_LOAD_PORT' => $config['port'] ?? '',
            'QUANTAFORGE_LOAD_USER' => $config['username'],
            'QUANTAFORGE_LOAD_PASSWORD' => $config['password'] ?? '',
            'QUANTAFORGE_LOAD_DATABASE' => $config['database'],
            'QUANTAFORGE_LOAD_SSL_CA' => $config['options'][\PDO::MYSQL_ATTR_SSL_CA] ?? '',
        ];
    }
