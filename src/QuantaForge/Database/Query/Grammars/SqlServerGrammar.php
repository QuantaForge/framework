...
namespace QuantaForge\Database\Query\Grammars;
...
class SqlServerGrammar extends Grammar
{
     ...
     *
     * @param  \QuantaForge\Database\Query\Builder  $query
     * @param  \QuantaForge\Database\Query\IndexHint  $indexHint
     * @return string
     */
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $sql .= 'using (values '.$parameters.') '.$this->wrapTable('quantaforge_source').' ('.$columns.') ';

        $on = collect($uniqueBy)->map(function ($column) use ($query) {
            return $this->wrap('quantaforge_source.'.$column).' = '.$this->wrap($query->from.'.'.$column);
        })->implode(' and ');
        ...
        if ($update) {
            $update = collect($update)->map(function ($value, $key) {
                return is_numeric($key)
                    ? $this->wrap($value).' = '.$this->wrap('quantaforge_source.'.$value)
