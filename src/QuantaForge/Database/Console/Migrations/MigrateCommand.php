...
namespace QuantaForge\Database\Console\Migrations;
...
use QuantaForge\Database\SQLiteDatabaseDoesNotExistException;
use QuantaForge\Database\SqlServerConnection;
use PDOException;
use Throwable;
...
class MigrateCommand extends BaseCommand implements Isolatable
{
     ...
     *
     * @var \QuantaForge\Database\Migrations\Migrator
     */
    protected $migrator;
     ...
     *
     * @param  \QuantaForge\Database\Migrations\Migrator  $migrator
    ...
    {
        if ($this->quantaforge['config']->get("database.connections.{$connection->getName()}.database") !== $connection->getDatabaseName()) {
            return false;
        }
        ...
        try {
            $this->quantaforge['config']->set("database.connections.{$connection->getName()}.database", null);

            $this->quantaforge['db']->purge();
            ...
            $this->quantaforge['config']->set("database.connections.{$connection->getName()}.database", $connection->getDatabaseName());
        }
    }
