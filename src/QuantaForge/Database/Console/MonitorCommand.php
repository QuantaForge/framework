...
use QuantaForge\Database\Events\DatabaseBusy;
...
class MonitorCommand extends DatabaseInspectionCommand
{
     ...
     *
     * @param  \QuantaForge\Database\ConnectionResolverInterface  $connection
    ...
    protected function parseDatabases($databases)
    {
        return collect(explode(',', $databases))->map(function ($database) {
            if (! $database) {
                $database = $this->quantaforge['config']['database.default'];
            }
     ...
     *
     * @param  \QuantaForge\Support\Collection  $databases
     * @return void
     */
