...
use QuantaForge\Http\Client\Factory as Http;
...
class DocsCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Access the QuantaForge documentation';
     ...
     *
     * @param  \QuantaForge\Http\Client\Factory  $http
    ...
    {
        if ($this->isSearching()) {
            return "https://quantaforge.com/docs/{$this->version()}?".Arr::query([
                'q' => $this->searchQuery(),
            ]);
        }

        return with($this->page(), function ($page) {
            return trim("https://quantaforge.com/docs/{$this->version()}/{$page}#{$this->section($page)}", '#/');
        });
    }
    ...
    /**
     * Fetch the documentation index from the QuantaForge website.
     *
     * @return \QuantaForge\Http\Client\Response
     */
    protected function fetchDocs()
    {
        return $this->http->get("https://quantaforge.com/docs/{$this->version()}/index.json");
    }
    ...
    {
        return Str::before($this->version ?? $this->quantaforge->version(), '.').'.x';
    }
