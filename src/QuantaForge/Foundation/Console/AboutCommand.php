...
class AboutCommand extends Command
{
        ...
        collect(static::$data)
            ->map(fn ($items) => collect($items)
                ->map(function ($value) {
                    ...
                    if (is_string($value)) {
                        $value = $this->quantaforge->make($value);
                    }

                    return collect($this->quantaforge->call($value))
     ...
     *
     * @param  \QuantaForge\Support\Collection  $data
     * @return void
     */
    ...
    protected function gatherApplicationInformation()
    {
        static::addToSection('Environment', fn () => [
            ...
            'QuantaForge Version' => $this->quantaforge->version(),
            'PHP Version' => phpversion(),
            'Composer Version' => $this->composer->getVersion() ?? '<fg=yellow;options=bold>-</>',
            'Environment' => $this->quantaforge->environment(),
            'Debug Mode' => config('app.debug') ? '<fg=yellow;options=bold>ENABLED</>' : 'OFF',
            'URL' => Str::of(config('app.url'))->replace(['http://', 'https://'], ''),
            'Maintenance Mode' => $this->quantaforge->isDownForMaintenance() ? '<fg=yellow;options=bold>ENABLED</>' : 'OFF',
        ]);

        static::addToSection('Cache', fn () => [
            'Config' => $this->quantaforge->configurationIsCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Events' => $this->quantaforge->eventsAreCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Routes' => $this->quantaforge->routesAreCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Views' => $this->hasPhpFiles($this->quantaforge->storagePath('framework/views')) ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
        ]);
