...
use QuantaForge\Routing\ViewController;
...
class RouteListCommand extends Command
{
    ...
    protected function isFrameworkController(Route $route)
    {
        return in_array($route->getControllerClass(), [
            '\QuantaForge\Routing\RedirectController',
            '\QuantaForge\Routing\ViewController',
        ], true);
    }
     ...
     *
     * @param  \QuantaForge\Support\Collection  $routes
     * @return string
     */
    ...
    protected function formatActionForCli($route)
    {
        ...
        $rootControllerNamespace = $this->quantaforge[UrlGenerator::class]->getRootControllerNamespace()
            ?? ($this->quantaforge->getNamespace().'Http\\Controllers');
