...
class StorageLinkCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($this->links() as $link => $target) {
            ...
            if (is_link($link)) {
                $this->quantaforge->make('files')->delete($link);
            }

            if ($relative) {
                $this->quantaforge->make('files')->relativeLink($target, $link);
            } else {
                $this->quantaforge->make('files')->link($target, $link);
            }
    ...
    {
        return $this->quantaforge['config']['filesystems.links'] ??
