...
use QuantaForge\Foundation\Exceptions\RegisterErrorViewPaths;
...
class DownCommand extends Command
{
    ...
    {
        try {
            if ($this->quantaforge->maintenanceMode()->active()) {
                $this->components->info('Application is already down.');

                return 0;
            }

            $this->quantaforge->maintenanceMode()->activate($this->getDownFilePayload());
            ...
            $this->quantaforge->get('events')->dispatch(new MaintenanceModeEnabled());
    ...
    {
        try {
            return $this->quantaforge->make(PreventRequestsDuringMaintenance::class)->getExcludedPaths();
        } catch (Throwable) {
            return [];
        }
    }
