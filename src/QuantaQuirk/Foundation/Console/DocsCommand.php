...
use QuantaQuirk\Http\Client\Factory as Http;
...
class DocsCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Access the QuantaQuirk documentation';
     ...
     *
     * @param  \QuantaQuirk\Http\Client\Factory  $http
    ...
    {
        if ($this->isSearching()) {
            return "https://quantaquirk.com/docs/{$this->version()}?".Arr::query([
                'q' => $this->searchQuery(),
            ]);
        }

        return with($this->page(), function ($page) {
            return trim("https://quantaquirk.com/docs/{$this->version()}/{$page}#{$this->section($page)}", '#/');
        });
    }
    ...
    /**
     * Fetch the documentation index from the QuantaQuirk website.
     *
     * @return \QuantaQuirk\Http\Client\Response
     */
    protected function fetchDocs()
    {
        return $this->http->get("https://quantaquirk.com/docs/{$this->version()}/index.json");
    }
    ...
    {
        return Str::before($this->version ?? $this->quantaquirk->version(), '.').'.x';
    }
