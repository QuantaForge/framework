...
class StorageLinkCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($this->links() as $link => $target) {
            ...
            if (is_link($link)) {
                $this->quantaquirk->make('files')->delete($link);
            }

            if ($relative) {
                $this->quantaquirk->make('files')->relativeLink($target, $link);
            } else {
                $this->quantaquirk->make('files')->link($target, $link);
            }
    ...
    {
        return $this->quantaquirk['config']['filesystems.links'] ??
