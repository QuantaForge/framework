...
use QuantaQuirk\Routing\ViewController;
...
class RouteListCommand extends Command
{
    ...
    protected function isFrameworkController(Route $route)
    {
        return in_array($route->getControllerClass(), [
            '\QuantaQuirk\Routing\RedirectController',
            '\QuantaQuirk\Routing\ViewController',
        ], true);
    }
     ...
     *
     * @param  \QuantaQuirk\Support\Collection  $routes
     * @return string
     */
    ...
    protected function formatActionForCli($route)
    {
        ...
        $rootControllerNamespace = $this->quantaquirk[UrlGenerator::class]->getRootControllerNamespace()
            ?? ($this->quantaquirk->getNamespace().'Http\\Controllers');
