...
class AboutCommand extends Command
{
        ...
        collect(static::$data)
            ->map(fn ($items) => collect($items)
                ->map(function ($value) {
                    ...
                    if (is_string($value)) {
                        $value = $this->quantaquirk->make($value);
                    }

                    return collect($this->quantaquirk->call($value))
     ...
     *
     * @param  \QuantaQuirk\Support\Collection  $data
     * @return void
     */
    ...
    protected function gatherApplicationInformation()
    {
        static::addToSection('Environment', fn () => [
            ...
            'QuantaQuirk Version' => $this->quantaquirk->version(),
            'PHP Version' => phpversion(),
            'Composer Version' => $this->composer->getVersion() ?? '<fg=yellow;options=bold>-</>',
            'Environment' => $this->quantaquirk->environment(),
            'Debug Mode' => config('app.debug') ? '<fg=yellow;options=bold>ENABLED</>' : 'OFF',
            'URL' => Str::of(config('app.url'))->replace(['http://', 'https://'], ''),
            'Maintenance Mode' => $this->quantaquirk->isDownForMaintenance() ? '<fg=yellow;options=bold>ENABLED</>' : 'OFF',
        ]);

        static::addToSection('Cache', fn () => [
            'Config' => $this->quantaquirk->configurationIsCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Events' => $this->quantaquirk->eventsAreCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Routes' => $this->quantaquirk->routesAreCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Views' => $this->hasPhpFiles($this->quantaquirk->storagePath('framework/views')) ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
        ]);
