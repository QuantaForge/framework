...
use QuantaQuirk\Tests\Database\Fixtures\Enums\Foo;
use Mockery as m;
...
class DatabaseMySqlSchemaGrammarTest extends TestCase
{
    ...
    public function testAddingForeignID()
    {
        ...
        $this->assertSame([
            'alter table `users` add `foo` bigint unsigned not null, add `company_id` bigint unsigned not null, add `quantaquirk_idea_id` bigint unsigned not null, add `team_id` bigint unsigned not null, add `team_column_id` bigint unsigned not null',
    ...
    public function testAddingForeignUuid()
    {
        ...
        $this->assertSame([
            'alter table `users` add `foo` char(36) not null, add `company_id` char(36) not null, add `quantaquirk_idea_id` char(36) not null, add `team_id` char(36) not null, add `team_column_id` char(36) not null',
            'alter table `users` add constraint `users_company_id_foreign` foreign key (`company_id`) references `companies` (`id`)',
            'alter table `users` add constraint `users_quantaquirk_idea_id_foreign` foreign key (`quantaquirk_idea_id`) references `quantaquirk_ideas` (`id`)',
            'alter table `users` add constraint `users_team_id_foreign` foreign key (`team_id`) references `teams` (`id`)',
            'alter table `users` add constraint `users_team_column_id_foreign` foreign key (`team_column_id`) references `teams` (`id`)',
